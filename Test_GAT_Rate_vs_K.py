...
UE_list = [3, 4, 5, 6, 7, 8]
for UE in UE_list:
    print("---------- Test case with UE = ", UE, " ----------")

    # Define system configuration
    K_test = 6  # We evaluate the scalability of our model with the model trained with K=6.
    M = 16  # Number of BS antennas
    scenario = 'Raytracing' + '_K=' + str(UE) + '/'
    ...
    with torch.no_grad():
        ...
        test_model_file = dirs + 'GAT_Train_with_' + 'L=' + str(L) + '_B=' + str(B) + '_SNR=' + str(SNR_dB) + '_K=' + \
                          str(K_test) + '.pt'
        ...
        for x_test in test_loader:
            ...
            # Perform inference
            W, vq_loss, rate_list = test_model(x_test_batch, train_mode=False)

            # Move results to CPU to free GPU memory
            W = W.to('cpu').detach().numpy()  # B x M x K
            vq_loss = vq_loss.to('cpu').detach().numpy()
    ...
    rate_file = res_dirs + 'SumRate_L=' + str(L) + '_B=' + str(B) + '_SNR=' + str(SNR_dB) + '_K=' + str(UE) + '_GAT.mat'
    sio.savemat(rate_file, {'data': sum_rate})

    BF_file = res_dirs + 'W_L=' + str(L) + '_B=' + str(B) + '_SNR=' + str(SNR_dB) + '_K=' + str(UE) + '_GAT.mat'
